...
export class CardCollection {
   ...
   * @returns {void}
   */
  public addCard(card: Card, callback: (error: Error | null, result: string) => void): void {
    // Si la carta ya existe en la colección, se llama al callback con un mensaje de error
    if (this.collection.some(c => c.id === card.id)) {
      callback(null, chalk.red(`La carta con ID ${card.id} ya existe en la colección de ${this.user}.`));
    } else { // Si no existe
      // Se añade la carta a la colección
      this.collection.push(card);
      // Se escribe la carta en un fichero JSON en el directorio del usuario de forma asíncrona
      fs.writeFile(this.getCardFilePath(card.id), JSON.stringify(card, null, 2), (err) => {
        if (err) { // Si hay un error, se llama al callback con el error
          callback(err, '');
        } else { // Si no hay error, se llama al callback con un mensaje de éxito
          callback(null, chalk.green(`Nueva carta añadida a la colección de ${this.user}.`));
        }
      });
    }
  }
   ...
   * @returns {void}
   */
  public removeCard(id: number, callback: (error: Error | null, result: string) => void): void {
    // Se busca el índice de la carta en la colección
    const cardIndex = this.collection.findIndex(c => c.id === id);
    // Si no se encuentra, se llama al callback con un mensaje de error. La carta no existe.
    if (cardIndex === -1) {
      callback(null, chalk.red(`La carta con ID ${id} no existe en la colección de ${this.user}.`));
    } else { // Si se encuentra
      // Se elimina la carta de la colección
      this.collection.splice(cardIndex, 1);
      // Se elimina el fichero de la carta de forma asíncrona
      fs.unlink(this.getCardFilePath(id), (err) => {
        if (err) { // Si hay un error, se llama al callback con el error
          callback(err, '');
        } else { // Si no hay error, se llama al callback con un mensaje de éxito
          callback(null, chalk.green(`Carta eliminada de la colección de ${this.user}.`));
        }
      });
    }
  }
